...
void play_track()
{
    ...
    musicPlayer.startPlayingFile(files[current_file]);
...
void set_volume()
{
    ...
    Serial.print("Volume set to level: ");
...
void loop()
{
	if (Serial.available()) {
        ...
        switch (c) {
            ...
            case 'n':
                current_file = (current_file + 1) % FILES_COUNT;
                play_track();
                break;

            case 'p':
                if (current_file == 0) {
                    current_file = FILES_COUNT - 1;
                } else {
                    --current_file;
                }
                play_track();
            ...
            case '-':
                if (current_volume < MAX_VOL_VALUE) {
                    ++current_volume;
                    set_volume();
                }
                break;
